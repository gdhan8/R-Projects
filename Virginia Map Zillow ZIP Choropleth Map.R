...
dest = paste0("~/Desktop/R Projects/Home Data Project/", "Zip_zhvi_uc_sfrcondo_tier_0.33_0.67_sm_sa_month_",Sys.Date(),".csv")
download.file(url, dest)

zhvi<-read.csv(dest)
zori<-read.csv("~/Desktop/R Projects/Home Data Project/Zip_ZORI_AllHomesPlusMultifamily_SSA-1.csv")
...
labels <- sprintf(
  ...
  prettyNum(zipcode_shapefile@data$X2021.08.31, big.mark=","),
...
zhvi_zc_map <- zipcode_shapefile %>% 
  ...
  setView(#####
          ...
          lat = 38.035556, lng = -78.503333, zoom = 9) %>% # Charlottesville, Va
